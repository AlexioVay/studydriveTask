...
function cleanLine($line) {
	...
	// strip out last column (ec: kundenreferenz/ cc:ursprünglicher betrag)
...
if (strpos($html_, 'Letzte Anmeldung:') !== false) {
	echo "<strong>Login OK.</strong><br>";
} else {
...
foreach ($dom_->find('table[id=gruppe-0] tr') as $k => $row) {
...
foreach ($accounts as $account) {
    $accountNr = preg_replace('/\s+/', '', $account['nr']);
    if ($accountNr != $ktonr) continue;

	$cnt = 0;
	$lines = explode("\n", $account['csv']);

	$exists = file_exists($file = __DIR__ . '/data/'.$accountNr.'.csv');
